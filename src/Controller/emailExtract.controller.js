...
export const emailExtract = warpAsync(async (req, res, next) => {
  ...
  for (let domain of domainArray) {
    ...
    try {
      ...
      console.error(
        `❌ Error fetching HTML from ${modifyDomain}:`,
        error.message
      );
      continue;
    }
    ...
    if (emailsData[domain].emails.length === 0) {
      ...
      for (const link of emailsData[domain].links) {
        ...
        emailsData[domain].emails =
          emailsData[domain].emails.concat(linkedEmails);
      }
      ...
      if (emailsData[domain].emails.length === 0) {
        throw new ExpressError(
          404,
          "Error occurred from DB",
          "No report exists with the provided id",
          "none"
        );
      }
    }
